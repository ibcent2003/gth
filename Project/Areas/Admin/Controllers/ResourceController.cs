...
namespace Project.Areas.Admin.Controllers
{
    ...
    public class ResourceController : Controller
    {
        ...
        public PartialViewResult Nav()
        {
            ...
            try
            {
                var GetUser = swdb.Users.Where(x => x.UserName == User.Identity.Name).FirstOrDefault();

                List<string> roleN = System.Web.Security.Roles.GetRolesForUser(User.Identity.Name).ToList();

                
                List<Guid> role = swdb.Roles.Where(x => roleN.Contains(x.RoleName)).Select(x => x.RoleId).ToList();
        ...
        public virtual ActionResult ResourceToRole(string Id)
        {
            ...
            model.Id = Id;
            model.AllRoles = (from a in swdb.Roles orderby a.RoleName select a).AsEnumerable().Select(x => new SelectListItem() { Value = x.RoleId.ToString(), Text = x.RoleName }).ToList();
            if (Id == null || Id == "")
            {
        ...
        public virtual ActionResult GrantResourceToRole(string roleId, string Resource)
        {
            ...
            var guidRoleId = Guid.Parse(roleId);
            var roleName = (from r in swdb.Roles where r.RoleId == guidRoleId select r.RoleName).FirstOrDefault();
