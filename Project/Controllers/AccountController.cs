        ...
        public virtual ActionResult Login(LogOnViewModel model, string returnUrl)
        {
            if (ModelState.IsValid)
            {
                if (authenticationService.LogOn(model.UserName, model.Password, model.RememberMe))
                {
                    ...
                    {
                        return RedirectToAction("Classic", "Home", new { area = "" });
                    }
                }
                else
                {
        ...
        public virtual ActionResult ForgotPassword(ForgotPasswordViewModel model)
        {
            try
            {
                ...
                else
                {
                    ...
                    {
                        ModelState.AddModelError("", mpe.Message);
                        return RedirectToAction("ForgotPassword");
                    }
                }
                ...
                //var message = "Username: " + user.UserName + " Password: " + newPassword;
                //KeyValuePair<bool, string> resp = appServices.SendEmailToPersons("no-reply@nigeriatradehub.gov.ng", model.Email, ConfigSettings.SecurityGuardEmailSubject, message, DateTime.Now, null, null, null);
                //if (@resp.Key)
                //    throw new Exception(resp.Value);
            }
            catch (Exception ex)
            {
                Elmah.ErrorSignal.FromCurrentContext().Raise(ex);
                ModelState.AddModelError("", Settings.Default.GenericExceptionMessage);
                return RedirectToAction("ForgotPassword");
            }
